...
# Save the local gradient in the aux nets
...
@torch.no_grad()
def compute_loss_acc(net, data, target, criterion):
    pred = net(data)
    loss = criterion(pred, target)
    accs = accuracy(pred, target)
    
    # record the loss/acc in the last block
...
# Compute the local gradients in the auxnet
